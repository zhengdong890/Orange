...
class pageView extends React.Component{
	render(){	
        ...
        var pageConfig = {
            	'listRows' : value.data_page.listRows,  //每页显示的条数
            	'rollPage' : value.data_page.rollPage  //分页栏每页显示的页数   	
            };
        /*初次获取数据*/
        if(value.data_page.totalRows === ''){
        	dispatch(actions.getData(1,{'firstRow':0,'listRows':pageConfig.listRows}));
        }
        /*分页处理*/
        var totalPages = Math.ceil(value.data_page.totalRows/value.data_page.listRows);//总页数
        var nowPage    = value.data_page.nowPage;
        //当前页面大于总页面
        if(!totalPages && nowPage>totalPages){
          nowPage = totalPages;//当前页面设置为总页面
        }  
        var now_cool_page      = pageConfig.rollPage/2,
            now_cool_page_ceil = Math.ceil(now_cool_page);
        var pageData = {
        	    listRows : value.data_page.listRows,
        	    rollPage : pageConfig.rollPage,
        	    totalRows : value.data_page.totalRows,
