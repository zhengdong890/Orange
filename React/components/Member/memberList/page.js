...
class pageView extends React.Component{
	render(){	
        ...
        var data_page = value.data_page;
        /*初次获取数据*/
        if(data_page.totalRows === ''){
        	dispatch(actions.getData(1,{'firstRow':0,'listRows':data_page.listRows}));
        }
        /*分页处理*/
        var totalPages = Math.ceil(data_page.totalRows/data_page.listRows);//总页数
        var nowPage    = data_page.nowPage;
        //当前页面大于总页面
        if(!totalPages && nowPage>totalPages){
          nowPage = totalPages;//当前页面设置为总页面
        }  
        var now_cool_page      = data_page.rollPage/2,
            now_cool_page_ceil = Math.ceil(now_cool_page);
        var pageData = {
        	    listRows : data_page.listRows,
        	    rollPage : data_page.rollPage,
        	    totalRows : data_page.totalRows,
	    ...
	    var data           = this.props.data,
	        ...
	        listRows       = data.listRows,
	  	    option = listrow_config.map(function(v , index){
	  	    	var select = v == data.listRows ? true : false;
	            return <option key = {index} value = {v} selected={select}>{v}</option>;        	                 
		    });
	    return(
