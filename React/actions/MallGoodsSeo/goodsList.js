...
import cash from '../../js/MallGoodsSeo/goodsList';
...
let actions = {
    ...
    getData : (nowPage,parmers) => (dispatch) => {    	    	
    	...
    	if(!cash.tableData[nowPage]){
	        ...
	        $.post('/index.php/MallGoodsSeo/goodsList',parmers,function(res){	 
	        	 dispatch({
	        		 type    : 'get_goods_data',
	        		 nowPage : nowPage,
	        		 data    : res
    ...
    goodsSeoEdit : () => (dispatch) => {
       $.post('/index.php/MallGoodsSeo/goodsSeoUpdate',cash.fixedEditData,function(res){
    	   if(res.status){  
    		   if(res.id){
      	       	   dispatch({
    	       		   type  : 'goods_seo_add',
