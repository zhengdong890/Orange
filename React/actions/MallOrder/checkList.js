...
import cash from '../../js//MallOrder/checkList';
...
let actions = {
	...
	orderCheckRequest : () => (dispatch) => {
        $.post('/index.php/MallOrder/orderCheck',cash.order_check_data,function(res){	
        	 alert(res.msg);
        	 if(res.status){
        		 if(cash.order_check_data.is_check){
    ...
    getData : (nowPage,parmers) => (dispatch) => {
    	if(!cash.tableData[nowPage]){
	        ...
	        $.post('/index.php/MallOrder/getCheckList',parmers,function(res){	        	 
