...
abstract class Driver {
   ...
   /**
     ...
     * @return string
     */
    protected function parseOrder_sqlsrv($order) {
        if(is_array($order)) {
            $array   =  array();
            foreach ($order as $key=>$val){
                if(is_numeric($key)) {
        ...
        $field=explode('.',$order);
        if($order){
            $order=count($field)==1?$field[0]:'thinkphp.'.$field[1];                          
        }else{
            $order="rand()";
        }
        return !empty($order)?  ' ORDER BY '.$order:'';
    ...
    public function parseSql($sql,$options=array()){
        $sql   = str_replace(
            array('%TABLE%','%DISTINCT%','%FIELD%','%JOIN%','%WHERE%','%GROUP%','%HAVING%','%ORDER%','%SQLSRV_ORDER%','%LIMIT%','%UNION%','%LOCK%','%COMMENT%','%FORCE%'),
            array(
                ...
                $this->parseOrder_sqlsrv(!empty($options['order'])?$options['order']:''),
                $this->parseLimit(!empty($options['limit'])?$options['limit']:''),
