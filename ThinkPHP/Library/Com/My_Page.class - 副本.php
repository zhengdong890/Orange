...
class My_Page{
    ...
    public function __construct($totalRows, $listRows=20,$ajax=false){
        ...
        if($ajax){//ajax分页
            ...
            /* 基础设置 */
            $this->totalRows  = $totalRows; //设置总记录数
            $this->listRows   = empty($_GET['listRows_p'])?$listRows:$_GET['listRows_p'];  //设置每页显示行数
            $this->parameter  = empty($parameter) ? $_GET : $parameter;
    ...
    private function set_css(){
        ...
        $css="<style>
                ...
                .my_page .ul_$randnumber{width:auto;float:left;height:auto;border:1px solid #DDDDDD;border-radius:4px;margin-left:{$css_config['margin-left']}}
                .my_page .ul_$randnumber select{width:auto;height:24px;float:left;margin-left:10px;margin-right:10px;margin-top:3px}
                .my_page .ul_$randnumber input{width:40px;height:30px;border:0;float:left;border-left:1px solid #DDDDDD;}
    ...
    private function get_js(){
        if($this->is_ajax){//开启ajax模式的js
            $js_code=$this->js_code;
            $nowPage=$this->nowPage;
            $ajax_url=$this->ajax_url;
            $ul_class=".ul_{$this->randnumber}";//获取分页随机样式名字
            $obj=$this->update_dom;
            $js="<script>
                    $('$ul_class .go').click(function(){
                        var p=$('.my_page $ul_class input').val();
                        ajax_page(p);
                    })
                    $('$ul_class .set_listRows').change(function(){
                        var p=$(this).val();
                        ajax_page(p);
                    })
                    function ajax_page(p){
                        $.post('$ajax_url',{'p':p},function(res){
                            $('$obj').html(res['html']);
                            $('$ul_class').parent('.my_page').html(res['page']);
                            eval($js_code);
                        })
                    }
            </script>";
            ...
            $ul_class=".ul_{$this->randnumber}";//获取分页随机样式名字
            $js="<script>
                    $('.my_page $ul_class .go').click(function(){
                        var p=$('.my_page $ul_class input').val();
                        var url='$url'+'?p='+p;
                        window.location.href=url;
                    })
                    $('.my_page $ul_class .set_listRows').change(function(){
                        var listRows_p=$(this).val();
                        var url='$url'+'?p='+$nowPage+'&listRows_p='+listRows_p;
                        window.location.href=url;
                    })
                 </script>";
        }
    ...
    public function show(){
        ...
        if($this->totalPages > $this->rollPage && ($this->nowPage + $now_cool_page) < $this->totalPages){
            $the_end = '<a href="' . $this->url($this->totalPages) . '">' . $this->config['end'] . '</a>';
        }
        //统计
        ...
        if($this->is_ajax){
            return $css."<ul class='ul_{$this->randnumber}'>{$the_listRows}{$page_str}</ul>{$this->get_js()}";
        }else{
            return $css."<div class='my_page'><ul class='ul_{$this->randnumber}'>{$the_listRows}{$page_str}</ul></div>{$this->get_js()}";
        }
