...
use Think\Model;
class IntegratedSelectModel extends Model{
  protected $tableName='Integrated_select'; //关闭检测字段

  /**
   * 添加中标集成项目
   * @access public
   * @param  array $data_   集成项目信息 一维数组    
  ...
  public function selectAdd($data_){              
        ...
        $id = M('Integrated_select')->add($data);
        if($id === false){
           $result = array(
              'status' => 0,
              'msg'    => '添加失败'
   ...
   * 修改中标集成项目
   * @access public
   * @param  array $data_   中标集成项目信息 一维数组
   * @return array $result 执行结果
   */
  public function selectUpdate($data_){
      ...
      $id = M('Integrated_select')->save($data);
      if($id === false){
          $result = array(
              'status' => 0,
              'msg'    => '数据添加失败'
  ...
  public function selectDelete($id){ 
       ...
       $r = M('Integrated_select')->where(array('id'=>$id))->delete();
       if($r === false){
          $result = array(
              'status' => 0,
              'msg'    => '删除失败'
